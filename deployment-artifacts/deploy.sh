...
# helm install --name web -f TailwindTraders-Website/Deploy/helm/gvalues.yaml --set ingress.protocol=https --set ingress.tls.secretName=tt-letsencrypt-prod --set ingress.tls.hosts={$AKS_CLUSTER} --set image.repository=$containerRegistry/web --set image.tag=latest TailwindTraders-Website/Deploy/helm/web/
